  ...
  <teleport to='body'>
    ...
    <Transition name='start-menu-slide'>
      ...
      >
        <!-- 顶部用户状态栏 -->
        <div
          class='p-6 flex items-center justify-between bg-black/5 dark:bg-white/5 border-b border-black/5 dark:border-white/10'>
          <div class='flex items-center gap-3'>
            <div
              class='w-10 h-10 rounded-full border border-black/10 dark:border-white/20 overflow-hidden bg-gradient-to-br from-blue-500/20 to-purple-500/20'>
          ...
          <div class='flex gap-2'>
            <button @click='startTask(getAppById("settings"));close()'
            ...
            <button @click='logout'
                    class='p-2 hover:bg-red-500/10 dark:hover:bg-red-500/20 hover:text-red-600 dark:hover:text-red-500 rounded-lg transition-all'>
              <svg xmlns='http://www.w3.org/2000/svg' width='14' height='14' viewBox='0 0 24 24' fill='none'
                   stroke='currentColor' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'>
        ...
        <div class='flex-1 overflow-y-auto px-6 pb-4 custom-scrollbar'>
          <div class='mb-6'>
            <div class='flex items-center justify-between mb-4 px-2'>
              <span class='text-xs font-bold uppercase tracking-wider opacity-50'>{{ showAll ? '所有应用': '已固定'}}</span>
              <button @click='showAll=!showAll'
                      class='text-xs bg-black/5 dark:bg-white/10 px-2 py-1 rounded hover:bg-black/10 dark:hover:bg-white/20 transition-colors'>

                {{ showAll ? '已固定应用 > ': '所有应用 >'}}
              </button>
            </div>
            <div class='grid grid-cols-4 gap-2' v-if='!showAll'>
              <div
              ...
              >
                <div
                  class='w-12 h-12 flex items-center justify-center rounded-xl bg-white dark:bg-white/5 shadow-sm group-hover:shadow-md transition-shadow'>
                  <AppIcon :id='app.id' />
                </div>
          ...
          <div v-if='!keyword'>
            ...
            <div class='grid grid-cols-2 gap-4'>
              <div v-for='i in 4' :key='i'
                   class='flex items-center gap-3 p-2 rounded-lg hover:bg-black/5 dark:hover:bg-white/10 cursor-pointer transition-colors'>
...
import { appStore, getAppById } from '../stores/appStore.ts'
...
const filteredApps = computed(() => {
  if (!keyword.value) return appStore.apps.filter(app => app.fixed)
