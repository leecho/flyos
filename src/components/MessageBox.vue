  ...
  <Transition name="fade">
    <div v-if="state.isOpen"
      ...
      <div class="w-full max-w-[400px] bg-[#f3f3f3] dark:bg-[#2c2c2c] rounded-xl shadow-[0_20px_50px_rgba(0,0,0,0.3)]
                  border border-white/20 dark:border-white/5 overflow-hidden flex flex-col animate-scale-in">
        ...
        <div class="flex items-center justify-between px-4 py-3 select-none">
          <div class="flex items-center gap-2">
            ...
            <span class="text-xs font-medium text-gray-600 dark:text-gray-300">{{ state.title }}</span>
          </div>
        ...
        <div class="px-6 py-4 flex-1">
          ...
          <div v-if="state.type === 'prompt'" class="mt-4">
            <input
              ...
              ref="inputRef"
              type="text"
              class="w-full px-3 py-2 bg-white dark:bg-black/20 border border-gray-300 dark:border-white/10 border-b-blue-600 dark:border-b-blue-500
                     focus:outline-none text-sm rounded-t-sm transition-all dark:text-white"
        ...
        <!-- 操作按钮 -->
        <div class="px-6 py-4 bg-gray-50/50 dark:bg-white/5 flex flex-row-reverse gap-3">
          <button
