<script setup lang="ts">
  ...
  const getButtonStyle = (id: string) => {
    const state = installState[id]
    if (!state) return 'bg-accent text-white shadow-lg shadow-accent/20'
    if (state.status === 'installing') return 'bg-[var(--fly-border-system)] text-[var(--fly-text-secondary)]'
    if (state.status === 'installed') return 'bg-accent/10 text-accent'
    return ''
  }
</script>

<template>
  <div class="app-store-container h-full flex flex-col bg-[var(--fly-bg-primary)] text-[var(--fly-text-primary)] font-sans overflow-hidden">
    <div class="app-store-layout-wrapper flex-1 flex overflow-hidden min-h-0 relative">
      
      <!-- Desktop Sidebar -->
      <aside class="store-sidebar w-64 border-r border-[var(--fly-border-system)] bg-[var(--fly-bg-secondary)] flex flex-col p-4 shrink-0 overflow-y-auto no-scrollbar">
        <div class="flex items-center gap-3 px-3 py-6 mb-4">
          <div class="w-10 h-10 rounded-xl bg-accent shadow-lg shadow-accent/20 flex items-center justify-center text-white">
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"></path><polyline points="3.27 6.96 12 12.01 20.73 6.96"></polyline><line x1="12" y1="22.08" x2="12" y2="12"></line></svg>
          </div>
          <div class="min-w-0">
            <h2 class="text-sm font-black tracking-tight uppercase">应用商店</h2>
            <p class="text-[10px] opacity-40 font-bold tracking-wider uppercase">FlyOS Store</p>
          </div>
        </div>
      ...
      <!-- Main Content Area -->
      <main class="store-main flex-1 flex flex-col min-w-0 overflow-hidden">
        <header class="h-16 flex items-center justify-between px-6 border-b border-[var(--fly-border-system)] bg-[var(--fly-bg-glass)] backdrop-blur-md sticky top-0 z-10 shrink-0">
          <h2 class="text-base font-bold tracking-tight mobile-title hidden truncate mr-4 leading-none">{{ categories.find(c => c.id === activeCategory)?.name }}</h2>
          <div class="relative w-full max-w-xs ml-auto">
            <Search class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 opacity-30" />
            <input type="text" placeholder="搜索应用、游戏与插件"
              class="w-full bg-black/5 dark:bg-white/5 border border-transparent rounded-full py-2 pl-10 pr-4 text-[13px] outline-none focus:bg-[var(--fly-bg-primary)] focus:ring-2 focus:ring-accent/10 transition-all font-medium">
          </div>
        </header>

        <div class="flex-1 overflow-y-auto p-4 sm:p-8 scroll-smooth no-scrollbar">
          <!-- Banner -->
          <div v-if="activeCategory === 'all'" class="store-banner mb-10 relative rounded-[var(--fly-radius-lg)] overflow-hidden h-48 sm:h-72 group cursor-pointer shadow-2xl shadow-black/5 shrink-0">
            <div class="absolute inset-0 bg-accent brightness-90"></div>
            <div class="absolute inset-0 bg-gradient-to-br from-white/20 to-transparent"></div>
            <div class="absolute -right-20 -top-20 w-80 h-80 bg-white/10 rounded-full blur-[100px] animate-pulse"></div>
            <div class="relative h-full flex flex-col justify-end px-6 sm:px-12 py-8 sm:py-12 text-white">
              <div class="text-[10px] font-black bg-white/20 backdrop-blur-md w-fit px-2 py-0.5 rounded uppercase tracking-[0.2em] mb-4">编辑精选</div>
              <div class="text-3xl sm:text-5xl font-black mb-2 tracking-tighter leading-none">Pixel Studio Pro</div>
              <div class="text-xs sm:text-base opacity-90 max-w-lg font-medium leading-relaxed line-clamp-2">专为 FlyOS 设计的下一代像素绘图引擎，支持层级管理与云端同步。</div>
            </div>
          </div>

          <div class="mb-6 flex items-center justify-between px-1">
            <h3 class="text-[11px] font-bold uppercase tracking-widest opacity-40">发现更多</h3>
          </div>

          <!-- Apps Grid -->
          <div class="store-grid">
            <div v-for="app in filteredApps" :key="app.id" class="app-card bg-[var(--fly-bg-glass)] border border-[var(--fly-border-glass)] shadow-sm hover:shadow-xl hover:scale-[1.02] transition-all duration-300">
      ...
      <nav class="store-bottom-nav absolute bottom-0 left-0 right-0 h-16 bg-[var(--fly-bg-glass)] backdrop-blur-2xl border-t border-[var(--fly-border-system)] flex items-center justify-around px-2 z-20 hidden">
        <button v-for="cat in categories" :key="cat.id" @click="activeCategory = cat.id"
          class="flex flex-col items-center gap-1 transition-all flex-1 py-1"
          :class="activeCategory === cat.id ? 'text-accent font-bold scale-105' : 'opacity-40'">
...
/* Tablet & Intermediate */
@container app-store (max-width: 850px) {
  .store-grid {
...
@container app-store (max-width: 550px) {
  .store-sidebar {
